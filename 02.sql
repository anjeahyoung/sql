...
from employees
where manager_id in (100, 101, 201);--or을 나타낸다
...
select last_name
from employees
where last_name like 'S%';--char타입 검색에서 활용
...
select last_name
from employees
where last_name like 'K___'; -- _임의의 한글자를 말한다.
...
select last_name, job_id
from employees
where job_id like 'I_\_%' escape '\'; --escape 와일드 카드를 문자화 시킨다.
...
select last_name, manager_id
from employees
where manager_id is null; --is null 값이 null인 데이터를 찾는다.
...
select last_name, job_id
from employees
where job_id not in ('IT-PROG', 'SA_REP');
